...
function buildWaybackCandidates(directUrl: string) {
  ...
  // Prefer Wayback candidates first; Jina is not reliable for FC2 HTML (often 403 landing page rendered as 200)
  return [
    u,
...
async function tryFetchIndexHtml(): Promise<{ url: string; html: string; sourcesTried: string[] } | undefined> {
  ...
  const bases = [
    ...
    "http://vipkohaku.x.fc2.com/2018GW/",
  ];
  const pages = ["top.html", "index.html", "menu_entry.html", "menu_top.html", "index.htm", "top.htm"]; 
  const tried: string[] = [];
...
async function run() {
  ...
  if (!indexCandidate) {
    ...
    const summary = [{ index: "--", status: "error" as const, error: "Failed to resolve 2018-gw index page from known candidates.", sourcesTried: [] as string[] }];
    ...
    console.error("Could not locate 2018-gw index. Please update festival-urls.json with a resolvable URL or add HTML to catch/2018-gw.");
    return;
  }
