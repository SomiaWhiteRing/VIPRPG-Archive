...
interface TimemapRow {
  ...
  endtimestamp: string; // latest in group
  groupcount: number;
  uniqcount: number;
}
...
function buildWaybackUrl(ts: string, original: string, kind: "im_" | "id_" | "fw_" = "id_") {
  const u = original.startsWith("http") ? original : new URL(original, FC2_BASE).toString();
...
function parseIndex2018Summer(html: string, assetsBase: string): IndexEntry[] {
  const $ = cheerio.load(html);
  const entries: IndexEntry[] = [];
  const rows = $("#table2 tr");
  rows.each((_, tr) => {
    ...
    const cellHtml = workCell.html() || workCell.text() || "";
    const aParts = cellHtml.replace(/<br\s*\/?\s*>/gi, "\n").split(/\n/)
...
async function parseDetailGeneric(url: string | undefined): Promise<DetailEntry> {
  ...
  result.authorComment = findRowText("作者コメント") || findRowText("作者のコメント") || findRowText("備考");
  result.hostComment = findRowText("管理人コメント") || findRowText("主催コメント");
  const authorName = findRowText("作者：") || findRowText("作者");
  if (authorName) result.authorName = authorName;
  return result;
}

async function copyIcon(index: string, sources: string[]): Promise<string | undefined> {
...
function pickIndexHtml(rows: TimemapRow[]): TimemapRow | undefined {
  // prefer pages that look like index/top/menu_entry
  const candidates = rows.filter(r => /\/(top|index|menu_entry|menu_top)\.html$/i.test(r.original) || /\/(top|index)\.htm$/i.test(r.original));
  if (candidates.length === 0) return undefined;
  // choose the latest by endtimestamp
  return candidates.sort((a,b)=> (a.endtimestamp||a.timestamp).localeCompare(b.endtimestamp||b.timestamp)).slice(-1)[0];
}
...
function buildIconRowsByIndex(rows: TimemapRow[], kind: "fc2" | "geo") {
  const map = new Map<string, TimemapRow[]>();
  for (const r of rows) {
    const u = r.original;
    const m = kind === "fc2"
      ? u.match(/\/2018s\/entry0*(\d{1,3})icon\d*\.(png|jpe?g|gif|bmp)$/i)
      : u.match(/\/tkoolvip2018summer\/entry0*(\d{1,3})icon\d*\.(png|jpe?g|gif|bmp)$/i);
    if (m) {
      const idx = m[1].padStart(2, "0");
      const list = map.get(idx) ?? [];
      list.push(r);
...
function buildScreenshotRowsByIndex(rows: TimemapRow[], kind: "fc2" | "geo") {
  const map = new Map<string, TimemapRow[]>();
  for (const r of rows) {
    const u = r.original;
    const isUnder = kind === "fc2" ? /\/2018s\//i.test(u) : /\/tkoolvip2018summer\//i.test(u);
    if (!isUnder) continue;
    const isImage = /\.(png|jpe?g|gif|bmp)$/i.test(u) || /ss\d*$/i.test(u);
    if (!isImage) continue;
    if (/icon/i.test(u) || /counter/i.test(u)) continue;
    // infer index by NN in filename
    let m = u.match(/entry(\d{2,3})/i);
    if (!m) m = u.match(/(?:^|[^\d])(\d{2})(?:[^\d]|\.|$)/);
    if (m) {
      const idx = m[1].padStart(2, "0");
      const list = map.get(idx) ?? [];
      list.push(r);
...
async function run() {
  ...
  // Index HTML
  const indexCandidatesRows = fc2Tm.filter(r => /(\/top|\/index|\/menu_entry|\/menu_top)\.html?$/i.test(r.original));
  if (indexCandidatesRows.length === 0) {
  ...
  // try all index candidates from oldest to newest until success
  indexCandidatesRows.sort((a,b)=> (a.timestamp).localeCompare(b.timestamp));
  let indexUrl: string | undefined;
  let indexHtml: string | undefined;
  for (const r of indexCandidatesRows) {
    const cand = buildWaybackUrl(r.endtimestamp || r.timestamp, r.original, "fw_");
    try { indexHtml = await fetchText(cand); indexUrl = cand; break; } catch {}
  }
  if (!indexHtml || !indexUrl) {
    const indexOriginals = [
      new URL("index.html", FC2_BASE).toString(),
      new URL("top.html", FC2_BASE).toString(),
      new URL("menu_entry.html", FC2_BASE).toString(),
      new URL("menu_top.html", FC2_BASE).toString(),
    ];
    for (const orig of indexOriginals) {
      const cand = `https://web.archive.org/web/2/${orig}`;
      try { indexHtml = await fetchText(cand); indexUrl = cand; break; } catch {}
    }
  }
  if (!indexHtml || !indexUrl) throw new Error("Failed to fetch any index html snapshot");
  await fs.writeFile(path.join(CATCH_DIR, path.basename(new URL(indexUrl).pathname) || "index.html"), indexHtml, "utf8");
  await saveBannerFromIndex(indexUrl!, indexHtml);

  const entries = parseIndex2018Summer(indexHtml, indexUrl);

  const fc2DetailRows = buildDetailRowsByIndex(fc2Tm, "fc2");
  const geoDetailRows = buildDetailRowsByIndex(geoTm, "geo");
  const fc2IconRows = buildIconRowsByIndex(fc2Tm, "fc2");
  const geoIconRows = buildIconRowsByIndex(geoTm, "geo");
  const fc2SsRows = buildScreenshotRowsByIndex(fc2Tm, "fc2");
  const geoSsRows = buildScreenshotRowsByIndex(geoTm, "geo");
  ...
  for (const e of entries) {
    ...
    for (const r of (fc2DetailRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      detailCandidates.push(buildWaybackUrl(ts, r.original, "fw_"));
    }
    for (const r of (geoDetailRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      detailCandidates.push(buildWaybackUrl(ts, r.original, "fw_"));
    }
    ...
    // Icon candidates: try all rows, im_ then id_
    const iconCandidates: string[] = [];
    for (const r of (fc2IconRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      iconCandidates.push(buildWaybackUrl(ts, r.original, "im_"));
      iconCandidates.push(buildWaybackUrl(ts, r.original, "id_"));
    }
    for (const r of (geoIconRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      iconCandidates.push(buildWaybackUrl(ts, r.original, "im_"));
      iconCandidates.push(buildWaybackUrl(ts, r.original, "id_"));
    }
    if (e.iconUrl) iconCandidates.push(e.iconUrl);
    const iconLocal = await copyIcon(e.index, iconCandidates);

    // Screenshot candidates: union of detail parsed + both timemaps (im_/id_)
    const screenshotOrdered = new Set<string>();
    if (detail.screenshots) for (const u of detail.screenshots) screenshotOrdered.add(u);
    for (const r of (fc2SsRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      screenshotOrdered.add(buildWaybackUrl(ts, r.original, "im_"));
      screenshotOrdered.add(buildWaybackUrl(ts, r.original, "id_"));
    }
    for (const r of (geoSsRows.get(e.index) || [])) {
      const ts = r.endtimestamp || r.timestamp;
      screenshotOrdered.add(buildWaybackUrl(ts, r.original, "im_"));
      screenshotOrdered.add(buildWaybackUrl(ts, r.original, "id_"));
    }
    const screenshotResult = await copyScreenshots(e.index, Array.from(screenshotOrdered));

    const authorFinal = e.author || detail.authorName || "";
    const forumFinal = e.forumUrl || undefined;
