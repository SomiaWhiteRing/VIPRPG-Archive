...
const BASE_ORIGIN = "https://vipkohaku.x.fc2.com/2018s/";
...
async function preferLiveExternal(input?: string | null): Promise<string | undefined> {
  ...
  try {
    ...
    // treat fc2 festival host as internal; others (e.g., jbbs.shitaraba.net) try live
    const isInternal = /vipkohaku\.x\.fc2\.com$/i.test(u.hostname) || /vipkohaku\.x\.fc2\.com$/i.test(u.hostname);
    if (!isInternal) {
      const ok = await checkLive(unwrapped);
      if (ok) return unwrapped; // keep live link
    }
  } catch {}
...
function buildAssetMaps(rows: TimemapRow[]) {
  ...
  for (const r of rows) {
    const u = r.original;
    if (!u.includes("/2018s/")) continue;
    // detail e.g. /2018s/entry01.html
    const mDetail = u.match(/\/2018s\/entry(\d{2,3})\.html$/i);
    if (mDetail) {
    ...
    // icon: /2018s/entry/icon01.png or icon001.png
    const mIcon = u.match(/\/2018s\/entry\/icon0*(\d{1,3})\.(png|jpe?g|gif|bmp)$/i);
    if (mIcon) {
    ...
    // screenshots: under /2018s/entry/, image but not icon
    if (/\/2018s\/entry\//i.test(u) && /\.(png|jpe?g|gif|bmp)$/i.test(u) && !/\/entry\/icon/i.test(u) && !/counter/i.test(u)) {
...
function buildAssetMapsGeocities(rows: TimemapRow[]) {
  ...
  for (const r of rows) {
    const u = r.original;
    if (!/\/tkoolvip2018summer\//i.test(u)) continue;
    // detail: /tkoolvip2018summer/entryNN.html
    const mDetail = u.match(/\/tkoolvip2018summer\/entry(\d{2,3})\.html$/i);
    if (mDetail) {
      const idx = mDetail[1].padStart(2, "0");
      const prev = detailByIndex.get(idx);
      if (!prev || r.endtimestamp > prev.endtimestamp) detailByIndex.set(idx, r);
    ...
    // icon: entryNNicon(.png|.jpg|.gif...) with optional extra digit e.g., icon1
    const mIcon = u.match(/\/tkoolvip2018summer\/entry0*(\d{1,3})icon\d*\.(png|jpe?g|gif|bmp)$/i);
    if (mIcon) {
      const idx = mIcon[1].padStart(2, "0");
      const prev = iconByIndex.get(idx);
      if (!prev || r.endtimestamp > prev.endtimestamp) iconByIndex.set(idx, r);
    ...
    // screenshots: entryNNss(.png|.jpg...) or entryNNss1/ss2 (with or without extension)
    const mSs = u.match(/\/tkoolvip2018summer\/entry(\d{2,3})ss\d*\.?([a-z0-9]+)?$/i);
    if (mSs) {
...
async function run() {
  ...
  const { iconByIndex: geoIconByIndex, detailByIndex: geoDetailByIndex, ssByIndex: geoSsByIndex } = buildAssetMapsGeocities(geoRows);
  ...
  for (const e of entries) {
    ...
    const detailRow = detailByIndex.get(e.index) || geoDetailByIndex.get(e.index);
    ...
    for (const cand of iconCandidates) {
      iconLocal = await copyIcon(e.index, cand);
      if (iconLocal) break;
    }
    if (!iconLocal && e.iconUrl) {
      // last resort: try original direct link (no referer) per guidance
      try {
        iconLocal = await copyIcon(e.index, e.iconUrl);
      } catch {}
    }
    ...
    const work: WorkEntryOut = {
      ...
      no: e.no,
      title: e.title,
      author: e.author || detail.authorName || "",
      category: e.genre,
      engine: e.engine,
    ...
    // detailDisabled rule: hostComment, authorComment, ss all empty or missing
