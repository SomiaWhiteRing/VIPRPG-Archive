...
const FESTIVAL_ID = "2017-gw";
const FESTIVAL_SLUG = "2017-gw";
const BASE_URL = "https://viprpg2017gw.x.2nt.com";
const INDEX_URL = `${BASE_URL}/index.html`;
const MENU_URL = `${BASE_URL}/menu.html`;
const GAMES_URL = `${BASE_URL}/games.html`;
...
const SMALL_IMAGE_LIMIT = 100; // <100px considered icon/small, skip as screenshot
...
async function saveBanner() {
  try {
    ...
    await fs.writeFile(path.join(CATCH_DIR, "menu.html"), html, "utf8");
    const $ = cheerio.load(html);
    // Try the first banner image in menu page (b2017-1.png)
    const bannerSrc = $("img[src*='b2017']").first().attr("src") || $("img").first().attr("src");
    const bannerAbs = toAbsoluteUrl(bannerSrc, MENU_URL);
    if (!bannerAbs) return;
...
function parseIndex(html: string): IndexEntry[] {
  ...
  // The table has header rows, then one row per entry.
  const rows = $("table tr");
  rows.each((_, tr) => {
    const $tr = $(tr);
    const tds = $tr.find("td");
    if (tds.length < 7) return; // skip headers
    const noCell = tds.eq(0).text().trim();
    if (!/^\d{1,2}$/.test(noCell)) return;
    const num = noCell.padStart(2, "0");
    ...
    // Title is the larger font text, author is the smaller font
    const title = sanitizeWhitespace(workCell.find("font[size='3']").first().text()) ||
      sanitizeWhitespace(workCell.find("a").first().text()) || `Work ${index}`;
    const author = sanitizeWhitespace(workCell.find("font[size='2']").first().text());

    // Genre/Engine cell: text before <font color> is genre; inside <font> is engine
    const geCell = tds.eq(3);
    const genre = sanitizeWhitespace(geCell.clone().children("font").remove().end().text().replace(/\s*\n\s*/g, " ").split("\n")[0]);
    const engine = sanitizeWhitespace(geCell.find("font").text());
    ...
    entries.push({
      ...
      no: num,
      title,
      iconUrl: iconUrl ? toAbsoluteUrl(iconUrl, GAMES_URL) : undefined,
      author,
      genre,
      engine,
      streamingRaw,
      detailUrl: detailUrl ? toAbsoluteUrl(detailUrl, GAMES_URL) : undefined,
      forumUrl: forumUrl ? toAbsoluteUrl(forumUrl, GAMES_URL) : undefined,
      downloadUrl: downloadUrl ? toAbsoluteUrl(downloadUrl, GAMES_URL) : undefined,
    });
  });
  return entries;
}
...
async function parseDetail(url: string | undefined): Promise<DetailEntry> {
  ...
  try {
    ...
    // Screenshots: images under ../image/ss/*.png with optional onmouseover/out
    const img = $("img[src*='/ss/']").first();
    const src = img.attr("src");
    const onOver = img.attr("onmouseover");
    const onOut = img.attr("onmouseout");
    const baseHref = url; // resolve relative assets against actual page URL
    ...
    const findRowText = (label: string) => {
      // rows like: <tr><td colspan="2">【作者コメント】<br> ...</td></tr>
      const cell = $("td").filter((_, el) => $(el).text().trim().startsWith(`【${label}】`));
      ...
      // strip leading label brackets + optional colon/spaces
      const stripped = text.replace(new RegExp(`^【${label}】[\\s　]*`), "");
...
async function run() {
  ...
  // Save frameset and menu for traceability
  try {
  ...
  const listHtml = await fetchText(GAMES_URL);
  await fs.writeFile(path.join(CATCH_DIR, "games.html"), listHtml, "utf8");
  const entries = parseIndex(listHtml);
